...
int pause1ms() {
    ...
    // struct timespec remaining, request = { 0, 1000000 }; // 1ms
    struct timespec remaining, request = { 0, 500000000 }; // 0.5s
    // struct timespec remaining, request = { 1, 0 }; // 1s
    int ret = nanosleep(&request, &remaining);
    printf("Paused for: %lu ns, ret = %i\n\n", nanos() - timestamp, ret);
    return ret;
}
...
int readSortedFramesFromQueues(uint32_t number, int sockfd) {
    int result = 0;
    struct milcan_frame* frame;
    int i;
    uint32_t count = 0;
    ...
    for(uint8_t priority = 0; priority < MILCAN_ID_PRIORITY_COUNT; priority++) {
        ...
        do {
            ...
            if(frame != NULL) {
                ...
                sendcantosck(sockfd, &(frame->frame));
                free(frame);
                pause1ms();
            }
        } while(frame != NULL);
...
void changePriority() {
    ...
    if(res != 0) {
        LOGE(TAG, "WARNING: Unable to increase process priority. Code will run but timing will not be as good. To avoid this run as a SU.");
    }
    priority = getpriority(PRIO_PROCESS, 0);
    LOGI(TAG, "Application priority is %i", priority);
}
...
int main(int argc, char *argv[]) {
    ...
    timestamp = nanos();
    result = readSortedFramesFromQueues(NUM_TEST_FRAMES, sockfd);
    ...
    timestamp = nanos();
    result = readSortedFramesFromQueues(NUM_TEST_FRAMES - 5, sockfd);
    printf("Time to read: %lu ns\n\n", nanos() - timestamp);
    if(result != 0) {
        LOGE(TAG, "Fatal error encountered. Execution stopped.");
        exit(result);
