...
struct milcan_a {
    ...
    uint64_t syncTimer;         // Used to calculate when teh next sync frame is due to be sent.
    uint16_t sync_freq_hz;      // The frequency to send the sync frame
    uint64_t sync_time_ns;      // The period to send the sync frame in ns
...
extern struct milcan_a* milcan_open(uint8_t speed, uint16_t sync_freq_hz, uint8_t sourceAddress, uint8_t can_interface_type, char* address, uint16_t port);
